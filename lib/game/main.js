...
.defines(function(){
	...
	MyGame = ig.Box2DGame.extend({
		...
		init: function() {
			...
			var as = new ig.AnimationSheet( 'media/laser_tiles.png', 8, 8 );
			this.backgroundAnims = {'media/laser_tiles.png': {
	        	0: new ig.Animation( as, 0.2, [0, 3, 6, 6, 6, 6, 6, 6, 3, 0] ),
	        	1: new ig.Animation( as, 0.2, [1, 4, 7, 7, 7, 7, 7, 7, 4, 1] ),
	        	2: new ig.Animation( as, 0.2, [2, 5, 8, 8, 8, 8, 8, 8, 5, 2] ),
	        	9: new ig.Animation( as, 0.2, [9, 12, 15, 15, 15, 15, 15, 15, 12, 9] ),
	        	10: new ig.Animation( as, 0.2, [10, 13, 16, 16, 16, 16, 16, 16, 13, 10] ),
	        	11: new ig.Animation( as, 0.2, [11, 14, 17, 17, 17, 17, 17, 17, 14, 11] )
	   		 	}
			};
		...
		update: function() {		
			...
			if(!vector.cooldown){
				if(ig.input.pressed('mdown')){
					...
					console.log("x: " + vector.start.x + " y: " + vector.start.y);
				}
				if (ig.input.state('mdown')) {
					console.log("state is mdown");
		           if(vector.clickDown){
		           		console.log("state mdown and clickdown = true");
